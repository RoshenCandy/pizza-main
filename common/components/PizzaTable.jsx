...
const PizzaTable = () => {
  ...
  const getPizzas = () => {
    ...
    axios
      .get(
        `http://localhost:3000/api/pizza/all`
      )
      .then((response) => {
  ...
  const deletePizza = (id) => {
    axios
      .delete(`http://localhost:3000/api/pizza/${id}`)
      .then((response) => {
        console.log(response)
      });
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'category',
      width: '10%',
      render: category => categoriesValue.map(e => {
    ...
    },
    {
      ...
      dataIndex: 'types',
      width: '15%',
      render: type => type.map(e => typesValue.map(t => {
      ...
      render: (_, record) =>
        dataSource.length >= 1 ? (
          <div className='pizza-options'>
            <Button className='edit_btn' type='link'
              onClick={() => {
            ...
            <Popconfirm title="Видалити піцу?" className='popconfirm' onConfirm={() => {
